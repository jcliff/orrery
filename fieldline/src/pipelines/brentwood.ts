...
const INPUT_PATH = new URL('../../data/raw/brentwood/parcels.geojson', import.meta.url).pathname;
const OUTPUT_DIR = new URL('../../../chrona/public/data/brentwood', import.meta.url).pathname;

// Grid cell size for aggregation (~50m at Brentwood latitude)
...
async function main() {
  console.log('Processing Brentwood parcel data...');
  ...
  for (const feature of raw.features) {
    const yearStr = feature.properties.YR_HS_BLT;
    const year = yearStr ? parseInt(yearStr, 10) : 0;
    const useCategory = getUseCategory(feature.properties.DESC_);
  ...
  const globalMedian = sortedAll[Math.floor(sortedAll.length / 2)] || 2000;
  ...
  for (const feature of raw.features) {
    const { APN, YR_HS_BLT, DESC_, Address, TOT_AREA } = feature.properties;

    // Skip parcels without geometry
    if (!feature.geometry) continue;

    // Determine year
    const yearNum = YR_HS_BLT ? parseInt(YR_HS_BLT, 10) : 0;
    const hasKnownYear = yearNum >= 1800 && yearNum <= 2025;
    const useCategory = getUseCategory(DESC_);
