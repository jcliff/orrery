...
const INPUT_PATH = new URL('../../data/raw/pittsburg/parcels.geojson', import.meta.url).pathname;
const OUTPUT_DIR = new URL('../../../chrona/public/data/pittsburg', import.meta.url).pathname;

// Grid cell size for aggregation (~50m at Pittsburg latitude)
...
async function main() {
  console.log('Processing Pittsburg parcel data...');
  ...
  for (const feature of raw.features) {
    // Try YR_HS_BLT first, then YR_BUILT
    const yearStr = feature.properties.YR_HS_BLT || feature.properties.YR_BUILT;
  ...
  const globalMedian = sortedAll[Math.floor(sortedAll.length / 2)] || 1975;
  ...
  for (const feature of raw.features) {
    const { parcel_das, YR_HS_BLT, YR_BUILT, USE_DESC, Address, BLDG_SQFT } = feature.properties;

    // Skip parcels without geometry
    if (!feature.geometry) continue;

    // Determine year (prefer YR_HS_BLT over YR_BUILT)
    const yearStr = YR_HS_BLT || YR_BUILT;
    ...
    const clusterKey = getClusterKey(parcel_das || '', lng, lat);
    const blockId = getBlockId(parcel_das || '');
