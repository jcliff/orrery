/**
 * Lyon County parcel processing pipeline.
 * Processes parcel data with year built into visualization-ready format.
 * Lyon County includes Fernley, Dayton, and Yerington.
 */
import { readFile, writeFile, mkdir } from 'node:fs/promises';
import {
  getBlockId, getClusterKey, getCentroid, median, findDominant,
...
const INPUT_PATH = new URL('../../data/raw/lyon/parcels.geojson', import.meta.url).pathname;
const OUTPUT_DIR = new URL('../../../chrona/public/data/lyon', import.meta.url).pathname;

// Lyon County uses different field names
interface RawFeature {
  type: 'Feature';
  properties: {
    APN: string | number | null;
    PCONYR: number | null;  // Primary construction year
    PCMPYR: number | null;  // Completion year
    PHY_ADDR: string | null;
    YLDUSE: string | null;  // Land use
    YIMPRV: number | null;  // Improvement value (proxy for area)
  };
  geometry: { type: 'Polygon' | 'MultiPolygon'; coordinates: unknown };
}

async function main() {
  console.log('Processing Lyon County parcel data...');
  ...
  for (const feature of raw.features) {
    const year = feature.properties.PCONYR || feature.properties.PCMPYR;
    const use = feature.properties.YLDUSE || 'Unknown';
  ...
  const globalMedian = median(Object.values(yearsByUse).flat()) || 1970;
  ...
  for (const feature of raw.features) {
    ...
    const { APN, PCONYR, PCMPYR, PHY_ADDR, YLDUSE, YIMPRV } = feature.properties;
    const rawYear = PCONYR || PCMPYR;
    const hasKnownYear = rawYear && rawYear >= 1800 && rawYear <= 2025;
    const use = YLDUSE || 'Unknown';
    const year = hasKnownYear ? rawYear : (medianByUse[use] || globalMedian);
    const estimated = !hasKnownYear;
    const useLabel = getUseLabel(YLDUSE);
    ...
    detailedFeatures.push({
      ...
      properties: {
        ...
        color: getUseColor(YLDUSE),
      },
      geometry: feature.geometry,
    });

    const [lng, lat] = getCentroid(feature.geometry as { type: string; coordinates: unknown });
    ...
    addToCluster(cluster, lng, lat, useLabel, year, YIMPRV || 0, estimated);
  }
