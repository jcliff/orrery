...
// Grid cell size for aggregation (~50m at Lyon County latitude)
...
interface RawFeature {
  ...
  properties: {
    ...
    PCONYR: number | null;  // Original construction year
    PCMPYR: number | null;  // Composite construction year
    PHY_ADDR: string | null;
    YLDUSE: string | null;  // Land use code
    YLANDV: number | null;
    YIMPRV: number | null;
    YNETV: number | null;
  };
...
async function main() {
  ...
  for (const feature of raw.features) {
    const { APN, PCONYR, PCMPYR, YLDUSE, PHY_ADDR, YIMPRV } = feature.properties;
    ...
    const color = getUseColor(YLDUSE);
    ...
    detailedFeatures.push({
      ...
      properties: {
        ...
        use: useLabel,
        address: PHY_ADDR || '',
        area: YIMPRV || 0,  // Use improvement value as proxy for area
        startTime,
        color,
      },
