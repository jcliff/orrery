/**
 * Washoe County parcel processing pipeline.
 * Processes parcel data with year built into visualization-ready format.
 */
import { readFile, writeFile, mkdir } from 'node:fs/promises';
import {
  getBlockId, getGridKey, getClusterKey, getCentroid, median, findDominant,
...
async function main() {
  ...
  // First pass: collect year statistics by use type
  ...
  console.log(`\nYear coverage: ${withYear.toLocaleString()} (${((withYear / raw.features.length) * 100).toFixed(1)}%) with year, ${withoutYear.toLocaleString()} without`);

  // Calculate median years for estimation
