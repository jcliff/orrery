/**
 * Douglas County parcel processing pipeline.
 *
 * Processes geocoded assessor data into visualization-ready format.
 * Uses Nevada Assessor Land Use Codes (same as Washoe).
 *
 * Note: Douglas County uses a different APN system in GIS vs assessor,
 * so we geocode addresses instead of joining by APN.
 */
import { readFile, writeFile, mkdir } from 'node:fs/promises';

const INPUT_PATH = new URL('../../data/raw/douglas/parcels-geocoded.geojson', import.meta.url).pathname;
const OUTPUT_DIR = new URL('../../../chrona/public/data/douglas', import.meta.url).pathname;

// Grid cell size for aggregation (~50m at Douglas County latitude)
...
async function main() {
  console.log('Processing Douglas County parcel data...');
  ...
  for (const feature of raw.features) {
    ...
    // Add detailed feature (with point geometry)
