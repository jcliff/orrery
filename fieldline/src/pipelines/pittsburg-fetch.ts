/**
 * Pittsburg parcel fetch pipeline.
...
const OUTPUT_DIR = new URL('../../data/raw/pittsburg', import.meta.url).pathname;
const SOURCE = getSource('pittsburg');
...
async function main() {
  ...
  cache.upsertFeatures(SOURCE.id, result.features, (f, i) =>
    geoJsonFeatureId(f, i, 'parcel_das')
  );
