...
const INPUT_PATH = new URL('../../data/raw/hayward/parcels.geojson', import.meta.url).pathname;
const OUTPUT_DIR = new URL('../../../chrona/public/data/hayward', import.meta.url).pathname;

// Grid cell size for aggregation (~50m at Hayward latitude)
...
// Hayward use description colors
...
function getUseCategory(useDesc: string | null): string {
  ...
  if (lower.includes('single family') || lower.includes('sfr') || lower.includes('1 family')) {
    return 'Single Family';
  }
  if (lower.includes('multi') || lower.includes('apartment') || lower.includes('condo') || lower.includes('duplex') || lower.includes('triplex') || lower.includes('fourplex')) {
    return 'Multi-Family';
  }
...
async function main() {
  console.log('Processing Hayward parcel data...');
  ...
  for (const feature of raw.features) {
    const yearStr = feature.properties.YEARBUILT;
    const year = yearStr ? parseInt(yearStr, 10) : 0;
    const useCategory = getUseCategory(feature.properties.USEDESCRIPTION);
  ...
  const globalMedian = sortedAll[Math.floor(sortedAll.length / 2)] || 1965;
  ...
  for (const feature of raw.features) {
    const { APN_GIS, YEARBUILT, USEDESCRIPTION, P_HouseNum, P_Street_Name, BUILDINGAREA } = feature.properties;

    // Skip parcels without geometry
    if (!feature.geometry) continue;

    // Determine year
    const yearNum = YEARBUILT ? parseInt(YEARBUILT, 10) : 0;
    const hasKnownYear = yearNum >= 1800 && yearNum <= 2025;
    const useCategory = getUseCategory(USEDESCRIPTION);
    ...
    const address = [P_HouseNum, P_Street_Name].filter(Boolean).join(' ');
    ...
    const clusterKey = getClusterKey(APN_GIS || '', lng, lat);
    const blockId = getBlockId(APN_GIS || '');
    ...
    cluster.totalArea += BUILDINGAREA || 0;
