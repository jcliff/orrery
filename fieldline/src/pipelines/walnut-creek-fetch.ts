/**
 * Walnut Creek parcel fetch pipeline.
...
const OUTPUT_DIR = new URL('../../data/raw/walnut-creek', import.meta.url).pathname;
const SOURCE = getSource('walnut-creek');
...
async function main() {
  ...
  cache.upsertFeatures(SOURCE.id, result.features, (f, i) =>
    geoJsonFeatureId(f, i, 'apn_pt_APN')
  );
