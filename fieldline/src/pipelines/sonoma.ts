...
const INPUT_PATH = new URL('../../data/raw/sonoma/parcels.geojson', import.meta.url).pathname;
const OUTPUT_DIR = new URL('../../../chrona/public/data/sonoma', import.meta.url).pathname;

// Grid cell size for aggregation (~50m at Sonoma County latitude)
const GRID_SIZE = 0.0005;

// Extract block ID from APN (first 3 digits for Sonoma format)
...
interface RawFeature {
  ...
  properties: {
    APN: string;
    BuildingPrimaryYearBuilt: number | null;
    BuildingPrimaryEffectiveYear: number | null;
    UseCode: string | null;
    UseCodeDescription: string | null;
    UseCodeType: string | null;
    SitusFormatted1: string | null;
    SitusCity: string | null;
    LandSizeAcres: number | null;
    LandSizeSqft: number | null;
    BuildingPrimarySize: number | null;
    BuildingPrimaryStories: number | null;
    BuildingPrimaryUnitCount: number | null;
    BuildingPrimaryBedRooms: number | null;
    BuildingPrimaryBaths: number | null;
  };
  geometry: {
...
function getUseCategory(useDesc: string | null, useType: string | null): string {
  const desc = (useDesc || '').toLowerCase();
  const type = (useType || '').toLowerCase();

  if (desc.includes('single family') || desc.includes('sfr') || desc.includes('1 family') || type === 'residential') {
    return 'Single Family';
  }
  if (desc.includes('multi') || desc.includes('apartment') || desc.includes('condo') || desc.includes('duplex') || desc.includes('triplex') || desc.includes('fourplex')) {
    return 'Multi-Family';
  }
  if (desc.includes('commercial') || desc.includes('retail') || desc.includes('office') || desc.includes('store') || desc.includes('shopping') || type === 'commercial') {
    return 'Commercial';
  }
  if (desc.includes('industrial') || desc.includes('warehouse') || desc.includes('manufacturing') || type === 'industrial') {
    return 'Industrial';
  }
  if (desc.includes('agric') || desc.includes('farm') || desc.includes('vineyard') || desc.includes('orchard') || desc.includes('ranch') || type === 'agricultural') {
    return 'Agricultural';
  }
  if (desc.includes('public') || desc.includes('school') || desc.includes('government') || desc.includes('church') || desc.includes('hospital') || type === 'exempt') {
    return 'Public';
  }
...
async function main() {
  console.log('Processing Sonoma County parcel data...');
  ...
  for (const feature of raw.features) {
    const year = feature.properties.BuildingPrimaryYearBuilt || 0;
    const useCategory = getUseCategory(feature.properties.UseCodeDescription, feature.properties.UseCodeType);
  ...
  for (const feature of raw.features) {
    const { APN, BuildingPrimaryYearBuilt, BuildingPrimaryEffectiveYear, UseCodeDescription, UseCodeType, SitusFormatted1, SitusCity, BuildingPrimarySize } = feature.properties;

    // Skip parcels without geometry
    if (!feature.geometry) continue;

    // Determine year
    const yearNum = BuildingPrimaryYearBuilt || BuildingPrimaryEffectiveYear || 0;
    const hasKnownYear = yearNum >= 1800 && yearNum <= 2025;
    const useCategory = getUseCategory(UseCodeDescription, UseCodeType);
    ...
    cluster.totalArea += BuildingPrimarySize || 0;
