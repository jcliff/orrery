/**
 * Washoe County parcel fetch pipeline.
...
const OUTPUT_DIR = new URL('../../data/raw/washoe', import.meta.url).pathname;
const SOURCE = getSource('washoe');
...
async function main() {
  ...
  const result = await parallelFetch<GeoJSON.Feature>(fetcher, {
    concurrency: 4,
    batchSize: 2000,
    maxBatches: 250, // Full dataset ~165k parcels
    onProgress: (progress) => {
