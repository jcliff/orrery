/**
 * Nevada NHGIS places (cities/towns) processing pipeline.
 *
 * Processes downloaded NHGIS place data into visualization-ready GeoJSON.
 * Uses centroid points from the data (INTPTLAT, INTPTLON).
 *
 * Usage:
 *   pnpm --filter fieldline pipeline:nhgis-nevada-places
 */
...
const INPUT_DIR = new URL('../../data/raw/nhgis/nevada-places', import.meta.url).pathname;
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          await walk(fullPath);
        } else if (entry.name.endsWith('_place.csv')) {
...
async function parseCSV(csvPath: string): Promise<PlaceRecord[]> {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const placeName = nameIdx >= 0 ? values[nameIdx]?.replace(/"/g, '') : '';
    ...
    if (lat !== 0 && lon !== 0) {
      records.push({ gisjoin, year, placeName, totalPop, areaLand, lat, lon });
    }
  }
...
async function main() {
  console.log('NHGIS Nevada Places Processing Pipeline');
  console.log('========================================\n');

  // Find all place CSV files
  const csvFiles = await findCSVFiles(INPUT_DIR);
  console.log(`Found ${csvFiles.length} place CSV files\n`);

  if (csvFiles.length === 0) {
    console.error('Error: No place CSV files found. Run the extract pipeline first:');
  ...
  for (const csvFile of csvFiles) {
    ...
    if (records.length > 0) {
      console.log(`  ${fileName}: ${records.length} Nevada places`);
      allRecords.push(...records);
    }
  }

  console.log(`\nTotal: ${allRecords.length} place-year records\n`);
  ...
  console.log(`Created ${features.length} place features\n`);
  ...
  for (const [year, yearFeatures] of Array.from(byYear.entries()).sort((a, b) => a[0] - b[0])) {
    ...
    console.log(`  ${year}: ${yearFeatures.length} places, ${totalPop.toLocaleString()} total pop`);
  }
  ...
  const outputPath = join(OUTPUT_DIR, 'places.geojson');
