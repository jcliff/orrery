/**
 * Eastern US (Original 13 Colonies) NHGIS processing pipeline.
 *
 ...
 *
 * Usage:
 *   pnpm --filter fieldline pipeline:nhgis-eastern
 */
...
async function loadHistoricalBoundaries(year: number): Promise<Map<string, CountyBoundary>> {
  ...
  try {
    ...
    while (true) {
      ...
      if (!isEasternState(gisjoin)) continue;
    ...
    console.log(`  ${year}: Loaded ${boundaries.size} Eastern US county boundaries`);
  } catch (err) {
...
async function main() {
  console.log('NHGIS Eastern US Processing Pipeline');
  console.log('====================================\n');
  ...
  for (const csvFile of csvFiles) {
    ...
    if (records.length > 0) {
      console.log(`  ${fileName}: ${records.length} Eastern US records`);
...
function estimateAreaFromGeometry(geometry: GeoJSON.Geometry): number {
  ...
  // Helper to get bounds without spread operator (avoids stack overflow on large arrays)
  ...
  if (geometry.type === 'Polygon') {
    const { minLon, maxLon, minLat, maxLat } = getBounds(geometry.coordinates[0]);
    const width = (maxLon - minLon) * milesPerDegree * 0.85;
    const height = (maxLat - minLat) * milesPerDegree;
    return width * height * 0.6;
  } else if (geometry.type === 'MultiPolygon') {
    let totalArea = 0;
    for (const poly of geometry.coordinates) {
      const { minLon, maxLon, minLat, maxLat } = getBounds(poly[0]);
      const width = (maxLon - minLon) * milesPerDegree * 0.85;
      const height = (maxLat - minLat) * milesPerDegree;
