/**
 * Lyon County parcel fetch pipeline.
...
const OUTPUT_DIR = new URL('../../data/raw/lyon', import.meta.url).pathname;
const SOURCE = getSource('lyon');
...
async function main() {
  ...
  const result = await parallelFetch<GeoJSON.Feature>(fetcher, {
    concurrency: 4,
    batchSize: 2000,
    maxBatches: 50, // ~35k parcels expected
    onProgress: (progress) => {
