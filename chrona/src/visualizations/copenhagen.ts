...
export const copenhagenConfig: VisualizationConfig = {
  id: 'copenhagen',
  name: 'Copenhagen Development',

  center: [12.57, 55.68], // Central Copenhagen
  zoom: 12,
  ...
  sources: [
    {
      id: 'buildings',
      type: 'geojson',
      url: '/data/copenhagen/buildings.geojson',
      maxzoom: ZOOM_THRESHOLD,
    },
    {
      id: 'buildings-tiles',
      type: 'pmtiles',
      url: 'pmtiles:///data/copenhagen/buildings.pmtiles',
  ...
  title: {
    text: 'Copenhagen Urban Development',
    sumProperty: 'count',
  ...
  popup: {
    ...
    render: (props) => {
      ...
      if (props.count && (props.count as number) > 1) {
        ...
        const units = props.units as number;
        return `
          <strong>${count.toLocaleString()} buildings</strong><br/>
          ${units !== count ? `${units.toLocaleString()} households<br/>` : ''}
